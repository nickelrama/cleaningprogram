...
###
                        ...
                        data_trainee_tasks = [[sheet.cell_value(q, t) for t in range(sheet.ncols)] for q in range(sheet.nrows)]
...
def attendantselections(username):
        while True:
                try:
                        ...
                        data_attendant_tasks = [[sheet.cell_value(q, t) for t in range(sheet.ncols)] for q in range(sheet.nrows)]
                        ...
                        data_attendant_tasks_merged = [item for item in data_attendant_tasks_merged1 if item not in in_progress]
...
def staffselections(username):
        while True:
                try:
                        ...
                        data_staff_tasks = [[sheet.cell_value(q, t) for t in range(sheet.ncols)] for q in range(sheet.nrows)]
...
def seniorselections(username):
        while True:
                try:
                        ...
                        data_senior_tasks = [[sheet.cell_value(q, t) for t in range(sheet.ncols)] for q in range(sheet.nrows)]
...
def read_from_memory():
    ...
    num2 = (len(all_games))
    if num >= num2:
        print('Not enough games to distribute for this week, writing remaining games to worksheet, make note of them before reseting')
        for row in zip(all_games):
            ws.append(row)
